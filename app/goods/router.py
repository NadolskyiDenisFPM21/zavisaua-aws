...
from .schemas import GoodsCreate, GoodsRead, GoodsUpdate, PaginationGoods
...
@router.get('/all', response_model=List[GoodsRead])
@cache(expire=3600)
@version(1)
async def get_goods():
...
@router.get('', response_model=PaginationGoods)
@cache(expire=3600)
@version(1)
async def get_pagination(page: int = 1, page_size: int = 20):
    return await GoodsDAO.get_pagination_goods(page=page, page_size=page_size)